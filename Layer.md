...
사람이 직접 매긴 정답 라벨은 아직 없습니다. 다만 STEP4 다중 모델 비교 기능으로 확보한 신호들을 조합해, 정식 baseline이 나오기 전까지 쓸 잠정 기준선(초안)을 다음과 같이 잡습니다.

1. **기존 분류 대조(구현됨)** — 재결서 파일명 접두어(충돌·좌초·화재·인명사상 등, KMST가 접수 시 붙인 사고 "유형")를 1차 참고축으로 삼습니다. 사고 유형과 사고 "원인"은 서로 다른 분류축이라 정답/오답으로 채점할 수는 없지만, LLM이 붙인 라벨이 기존 분류와 방향성 있게 대응하는지(예: "화재/폭발" 라벨을 받은 문서가 실제로 "화재" 접두어 문서에 몰리는지)는 gui_web STEP4의 "기존 분류 → LLM 제안 라벨 흐름" 크로스탭으로 바로 확인할 수 있습니다.
2. **모델 간 합의(구현됨)** — Qwen2.5-3B/7B/14B, Llama-3.1-8B, OpenAI gpt-4o-mini 중 2개 이상이 같은 군집에 같은 라벨을 제안하면 "잠정 채택", 갈리면 "재검토 필요"로 잠정 등급을 매깁니다(gui_web의 "군집별 모델 간 라벨 일치도" 표). 계열·크기가 다른 모델들이 독립적으로 같은 결론에 도달했다는 건 그 군집 특징이 뚜렷하다는 신호로 해석합니다.
3. **사람 검증 샘플(다음 단계, 미착수)** — STEP1에서 썼던 것과 같은 방식(needs_review 30건 표본 검증, 12.5절 참고)으로, 군집당 5~6건씩 총 30건 내외를 사람이 직접 라벨링해 LLM 제안과의 실제 일치율을 계산합니다. 이게 진짜 정확도 baseline이고, 1·2번은 그 전까지의 잠정 대체재입니다.

1·2번 신호는 "그럴듯함"을 보여줄 뿐 정확도를 보장하지 않습니다 — 지금 나오는 결과는 여전히 3번(사람 검증) 이전의 초안입니다.
