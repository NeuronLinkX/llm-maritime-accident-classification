...
17. 임의로 내용을 생성하지 않는다.
18. 페르소나 생성을 중단한다.
19. corpus_validation_report.md에 실패 사유를 기록한다.
    5.2 코퍼스 매니페스트
...
21. 「해양사고의 조사 및 심판에 관한 법률」
22. 같은 법 시행령
23. 같은 법 시행규칙
24. 해당 법령의 위임을 받은 행정규칙
25. 기타 해양안전심판원 행정규칙
26. IMO CI Code 참고지침
...
10. 페르소나 1: 사실·증거 구조화 분석관
    ...
    10.5 수행업무
11. 사건번호와 재결기관을 추출한다.
12. 사고유형과 발생일시·장소를 추출한다.
13. 관련 선박과 행위자를 식별한다.
14. 사고 전·중·후 타임라인을 구성한다.
15. 재결서가 인정한 사실을 추출한다.
16. 당사자 주장과 증인 진술을 분리한다.
17. 객관적 기록과 물적 증거를 추출한다.
18. 전문가 의견과 감정 결과를 구분한다.
19. 서로 충돌하는 진술과 증거를 연결한다.
20. 누락되거나 확인되지 않은 정보를 표시한다.
21. 모든 추출 결과에 원문 위치를 연결한다.
22. 개인정보를 비식별화한다.
    10.6 사실 상태
...
23. 페르소나 2: 사고원인·법령 정합성 검증관
    ...
    UNKNOWN_FACTOR
    11.6 수행업무
24. 재결서가 명시한 원인을 추출한다.
25. 모델이 추가로 식별한 원인 후보를 분리한다.
26. 각 원인과 사실·증거를 연결한다.
27. 직접원인과 기여원인을 구분한다.
28. 사고 발생원인과 피해 확대 원인을 구분한다.
29. 원인의 시간적 선후관계를 확인한다.
30. 원인이 제거되었을 경우 결과가 달라졌을 가능성을 검토한다.
31. 반대 증거와 대체 원인을 검토한다.
32. 중복 원인을 병합하거나 계층화한다.
33. 관련 법령·행정규칙을 연결한다.
34. 국내 법령과 IMO 참고지침을 구분한다.
35. 증거 부족 또는 법령 충돌을 표시한다.
    11.7 인과관계 검증기준
...
33. 인간 전문가 검토조건
34. 품질검증 체크리스트
35. 실제 실행용 System Prompt
36. 실제 실행용 User Prompt Template
...
42. 10개 필수 법령·행정규칙 문서를 모두 읽었다.
43. IMO CI Code를 보조 지침으로만 분리했다.
44. 코퍼스 매니페스트와 검증보고서를 작성했다.
45. 역할이 분리된 페르소나 3개를 생성했다.
46. 각 페르소나의 실행용 프롬프트와 JSON 스키마를 생성했다.
47. 공식 원인제공비율과 분석용 원인기여도를 분리했다.
48. GoldSet 후보 검증정책을 작성했다.
49. 데이터 누출 방지정책을 작성했다.
50. 모든 산출물을 지정된 위치에 저장했다.
51. 최종 검증결과가 PASS다.
52. 각 페르소나의 수행업무·판단기준·오류조건·금지행동 항목마다 유형 A(SOURCE 근거) 또는 유형 B(연구설계 지정) 표기가 누락 없이 붙어 있다.
